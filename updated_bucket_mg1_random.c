...
static void open_bucket_period_csvs(void) {
    g_bucket_busy_csv = fopen("bucket_busy_periods.csv", "w");
    if (!g_bucket_busy_csv) { perror("fopen bucket_busy_periods.csv"); exit(1); }

    g_bucket_empty_csv = fopen("bucket_empty_periods.csv", "w");
    if (!g_bucket_empty_csv) { perror("fopen bucket_empty_periods.csv"); exit(1); }
...
static void bucket_note_arrival(uint8_t proto, uint32_t bucket_idx, uint64_t t_us, int bucket_was_empty) {
...
static void write_bucket_summary_csv(const char *csv_path) {
    ...
    if (!out) { perror("fopen bucket_mg1_summary.csv"); return; }
...
static void track_packet(const struct timeval *tv,
    ...
    if (!m->in_use) {
        ...
        bucket_note_arrival(proto, p, t_us, 1);
        ...
        /* New eligible flow arrival found a busy bucket.  For the M/G/1
           loss model, this is a rejected/lost arrival.  Under RC/WRC, the
           implementation may still replace the incumbent; both cases are
           recorded separately below. */
        bucket_note_arrival(proto, p, t_us, 0);
...
int main(int argc, char **argv) {
    ...
    write_bucket_summary_csv("bucket_mg1_summary.csv");
