 ...
 *********************************************************************/
 ...
 static int find_bucket(const flow_key_t *key, uint32_t h, int *found) {
    uint32_t p = h & (TABLE_SIZE - 1); //Retrieve the bucket by modding the hash by the size of the table - 1.
    if (!table[p].in_use) { //Check if the bucket in the hash table is not in use.
      *found = 0;
      return (int)p;
    }
