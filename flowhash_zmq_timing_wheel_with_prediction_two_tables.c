...
/*
 * CHANGE: dump_and_clear_main() no longer assumes a single global `table[]`.
 * We only remove from the timing wheel if:
 *   - the flow is UDP, AND
 *   - the pointer belongs to `table_udp[]` (UDP main table).
 */
...
/*
 * CHANGE: drop_and_clear_aux() no longer assumes a single global `aux[]`.
 * We remove from the wheel only for UDP aux (aux_udp[]).
 */
...
int main(int argc, char **argv) {
  ...
  /*
   * CHANGE: flush BOTH main tables (TCP + UDP).
