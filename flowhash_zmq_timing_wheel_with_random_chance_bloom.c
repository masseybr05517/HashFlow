 ...
 *
 *  NEW in this version (per your request):
 *    - Tracks (GROUND TRUTH, per-flow over the whole PCAP):
 *        * first_ts_us  (time first packet of the flow is seen)
 *        * hit30_ts_us  (time packet #30 of the flow is seen, if ever)
 *        * last_ts_us   (time last packet of the flow is seen = "flow concludes")
 *    - Exports these fields in the flow summary CSV:
 *        random_chance_flow_collision_summary.csv
 *
 *  Collision eligibility gating (as requested earlier):
 *    - TCP collision event counts/logs ONLY if challenger packet is PURE SYN
 *      (flags == TH_SYN, NOT SYN+ACK, not any other).
 *    - UDP collision event counts/logs ONLY if challenger flow is not in table
 *      AND it PASSES Bloom admission (definitely-not-seen -> inserted into bloom).
 *
 *  Outputs:
 *    - random_chance_collisions.csv   (event-level)
 *    - random_chance_flow_collision_summary.csv (flow-level ground truth + timing)
 *
 *  Usage:
 *    ./prog file.pcap [seed]
 *********************************************************************/
...
static inline int evict_50_50(void) {
  return rng32() < 0x80000000u;
}
...
static uint64_t st_cap_flushes = 0;       /* number of times we flushed a tracked flow at FLOW_CAP */

/* ================================================================= */
/*    Ground-truth per-flow packet counts + per-flow collision stats  */
/*    + NEW: first/hit30/last timestamps (us)                          */
/* ================================================================= */
...
typedef struct {
  ...
  uint32_t collisions;    /* how many times this flow appears in collision events (winner/loser) */
  uint32_t wins;
  uint32_t losses;

  /* NEW timing fields */
  uint64_t first_ts_us;   /* when packet #1 seen */
  uint64_t hit30_ts_us;   /* when packet #30 seen (0 if never) */
  uint64_t last_ts_us;    /* when last packet seen (flow "concludes") */
} gt_entry_t;
...
/* NEW: ground truth packet counting with timing */
static inline void gt_count_packet_with_time(const flow_key_t *k, const struct timeval *tv) {
  ...
  e->last_ts_us = t_us; /* always update */
}
...
static void write_flow_collision_summary_csv(const char *csv_path) {
  ...
  /* NEW column added: dur_30_to_last_us */
  fprintf(out,
  ...
  for (uint32_t i = 0; i < GT_SIZE; i++) {
    ...
    /* NEW: time from packet #30 to the last packet seen */
...
/* Decide if we can admit a UDP flow (Bloom gate).
   Returns 1 if allowed (definitely not seen -> inserts), 0 if refused (probably seen). */
                         ...
                         int tcp_pure_syn, uint16_t ip_len)
{
  ...
  /* ground truth counts + NEW timing (first/hit30/last) */
  ...
  if (!m->in_use) {
    ...
    /* ---- Eligibility gating BEFORE counting/logging ----
       TCP: only pure SYN can be a collision event.
       UDP: only if challenger passes Bloom admission can be a collision event. */
    if (proto == IPPROTO_TCP) {
      if (!tcp_pure_syn) return; /* not pure SYN => do not count/log collision */
    } else { /* UDP */
      if (!udp_admission_allowed(&key)) {
        /* Bloom refused => do not count/log collision; challenger is dropped */
        return;
      }
    }

    /* Now this truly qualifies as a "collision event" by your definition */
    st_collisions++;
    st_battles++;
...
int main(int argc, char **argv) {
  ...
  /* NOTE: this summary now includes first/hit30/last timestamps + durations */
