 ...
 *********************************************************************/
 ...
 typedef struct flow_entry {
   ...
   uint8_t ml_ready;   // 0 until count reaches FIRST_N
   uint8_t ml_pred;    // 1 if predicted long (>=40), else 0
 } flow_entry_t;
...
static void *sender_thread(void *arg) {
 ...
 static inline int coin_weighted_mixed(const flow_entry_t *e) {
  ...
  /* If you want an "escape hatch" to avoid permanent lock-in, set to 1 (percent).
   If you want purely ML-only with no randomness, set to 0. */
 #define ML_PROTECT_ESCAPE_PCT 0
 ...
 static int find_bucket(const flow_key_t *key, uint32_t h,
                       int *found, int *evict)
  {
     ...
     *found = 0;
     ...
     /* ML-based protection:
        - If incumbent is predicted long, do NOT evict it (reject contender).
        - Otherwise, evict incumbent and accept contender. */
     if (table[p].ml_ready && table[p].ml_pred) {

         *evict = 0;
         return -1;  /* reject contender */
     }
 
     /* incumbent not protected => evict incumbent */
     *evict = 1;
     return (int)p;
  }
...
static void write_to_csv(flow_entry_t *e)
 ...
 static void track_packet(const struct timeval *tv, uint32_t sip, uint32_t dip,
   ...
   if (e->count < FLOW_CAP) {
     ...
     e->count++;
     if (!e->ml_ready && e->count == FIRST_N) {
        e->ml_pred = ml_predict_long(e); // returns 1 if predicted >=40 else 0
        e->ml_ready = 1;
     }
   }
