...
typedef struct {
  ...
  uint64_t challenger_wins;            /* WRC replacement wins */
...
static void open_bucket_period_csvs(void) {
  g_bucket_busy_csv = fopen("wrc_bucket_busy_periods.csv", "w");
  if (!g_bucket_busy_csv) { perror("fopen wrc_bucket_busy_periods.csv"); exit(1); }

  g_bucket_empty_csv = fopen("wrc_bucket_empty_periods.csv", "w");
  if (!g_bucket_empty_csv) { perror("fopen wrc_bucket_empty_periods.csv"); exit(1); }
...
static void write_bucket_summary_csv(const char *csv_path) {
  ...
  if (!out) { perror("fopen wrc_bucket_mg1_summary.csv"); return; }
...
static void write_collisions_csv_from_bin(const char *bin_path, const char *csv_path) {
  ...
  if (!in) { perror("fopen wrc_collisions.bin"); return; }

  FILE *out = fopen(csv_path, "w");
  if (!out) { perror("fopen wrc_collisions.csv"); fclose(in); return; }
...
static void write_flow_collision_summary_csv(const char *csv_path) {
  FILE *out = fopen(csv_path, "w");
  if (!out) { perror("fopen wrc_flow_collision_summary.csv"); return; }
...
static void write_to_csv(flow_entry_t *e) {
  ...
  const char *fname = e->is_udp
    ? "wrc_flow_output_timing_wheel_udp.csv"
    : "wrc_flow_output_timing_wheel_tcp.csv";
                         ...
                         int tcp_pure_syn, uint16_t ip_len)
{
  ...
  if (!m->in_use) {
    ...
    /* New eligible flow arrival found a busy bucket. For the M/G/1
       loss model, this is a rejected/lost arrival. Under WRC, the
...
int main(int argc, char **argv) {
  ...
  g_colbin = fopen("wrc_collisions.bin", "wb");
  if (!g_colbin) { perror("fopen wrc_collisions.bin"); return 1; }
  ...
  write_collisions_csv_from_bin("wrc_collisions.bin", "wrc_collisions.csv");
  write_flow_collision_summary_csv("wrc_flow_collision_summary.csv");

  bucket_finalize_empty_periods(g_last_pcap_ts_us);
  write_bucket_summary_csv("wrc_bucket_mg1_summary.csv");
